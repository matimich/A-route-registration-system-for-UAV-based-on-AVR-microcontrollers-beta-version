	...
	if (res == FR_OK) {
		if (fp->flag & FA__WRITTEN) {	/* Has the file been written? */
			...
			if (res == FR_OK) {
				...
				tim = 1;//get_fattime();						/* Update updated time */
...
)
{
	...
	DIR dj;
	BYTE *dir, n;
	DWORD dsc, dcl, pcl, tim =1;// get_fattime();
	DEF_NAMEBUF;
...
)
{
	...
	n = 1;// get_fattime();						/* Use current time as VSN */
	if (fmt == FS_FAT32) {
...
)
{
	...
	return (bw == btw) ? 1 : '\0';	/* Return the result */
}
